...
%
%	This is  the latest version that will run on MATLAB 25.0
...
if strcmp(action,'initialize')
   ...
   if ~isempty(pph);
      ...
      if strcmp(answer,b1str)
         delete(pph);
         % Continue with initialization after deleting old figures
         % (removed return to allow initialization to proceed)
  ...
  if strcmp(comp,'PCWIN')
    ud.fontsize = 1.5*ud.ssize;  % Increased font size for better visibility
  else
    ud.fontsize = 1.5*ud.ssize;  % Increased font size for better visibility
  end
  ...
  % Ensure File menu exists (for MATLAB 2025 compatibility)
  if isempty(mefile)
    mefile = uimenu(ppset,'label','&File');
  end
  meedit = findobj(ppset,'label','&Edit');
  % Ensure Edit menu exists (for MATLAB 2025 compatibility)
  if isempty(meedit)
  ...
  child_menus = get(mefile,'child');
  if ~isempty(child_menus)
    set(child_menus,'vis','off');
  end
  ...
  set(mepset,'vis','on');
  set(meprev,'vis','on');
  set(meexp,'vis','on','separator','off');
   ...
   if (sud.flag == 1)
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       dud.ccwind = uicontrol('style','text',...
			      'horiz','left',...
			      'string',cstr,...
			      'visible','off');  % Hidden cursor position display
       ...
       set(findobj(fmenu,'label','Pa&ge Setup...'));
       set(findobj(fmenu,'label','Print Set&up...'));
       set(findobj(fmenu,'label','Print Pre&view...'));
       set(findobj(fmenu,'label','&Print...'));
       ...
       set(findobj(emenu,'label','Cu&t'));
       set(findobj(emenu,'label','&Copy'));
       set(findobj(emenu,'label','&Paste'));
       set(findobj(emenu,'label','Clea&r'));
       set(findobj(emenu,'label','&Select All'));
       set(findobj(emenu,'label','Copy &Figure'));
       set(findobj(emenu,'label','Copy &Options'));
       set(findobj(emenu,'label','F&igure Properties'));
       set(findobj(emenu,'label','&Axes Properties'));
       set(findobj(emenu,'label','C&urrent Object Properties'));
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       % Disabled cursor position display (WindowButtonMotionFcn)
...
elseif strcmp(action,'linear')
   ...
   if (~isempty(pplin))
      ...
      mequit = uimenu(fmenu,'label','Quit pplane8',...
	  'callback','pplane8(''quit'')','separator','on');
...
%  z0 = ppginput(1);
  ...
  infostr{5,1} = ['      ',sprintf('%.2f',A(1,1)),'   ',sprintf('%.2f',A(1,2))];
  infostr{6,1} = ['      ',sprintf('%.2f',A(2,1)),'   ',sprintf('%.2f',A(2,2))];
  infostr{7,1} = '';
  infostr{8,1} = 'The eigenvalues and eigenvectors are:';
  % Format eigenvalue 1 (handle complex numbers)
  if imag(D(1,1)) == 0
    eig1_str = sprintf('%.2f', real(D(1,1)));
  else
    eig1_str = sprintf('%.2f%+.2fi', real(D(1,1)), imag(D(1,1)));
  end
  % Format eigenvector 1 (handle complex numbers)
  if imag(V(1,1)) == 0
    v1_1_str = sprintf('%.2f', real(V(1,1)));
  else
    v1_1_str = sprintf('%.2f%+.2fi', real(V(1,1)), imag(V(1,1)));
  end
  if imag(V(2,1)) == 0
    v1_2_str = sprintf('%.2f', real(V(2,1)));
  else
    v1_2_str = sprintf('%.2f%+.2fi', real(V(2,1)), imag(V(2,1)));
  end
  infostr{9,1} = ['     ',eig1_str,'    (',v1_1_str,', ',v1_2_str,')'];
  
  % Format eigenvalue 2 (handle complex numbers)
  if imag(D(2,2)) == 0
    eig2_str = sprintf('%.2f', real(D(2,2)));
  else
    eig2_str = sprintf('%.2f%+.2fi', real(D(2,2)), imag(D(2,2)));
  end
  % Format eigenvector 2 (handle complex numbers)
  if imag(V(1,2)) == 0
    v2_1_str = sprintf('%.2f', real(V(1,2)));
  else
    v2_1_str = sprintf('%.2f%+.2fi', real(V(1,2)), imag(V(1,2)));
  end
  if imag(V(2,2)) == 0
    v2_2_str = sprintf('%.2f', real(V(2,2)));
  else
    v2_2_str = sprintf('%.2f%+.2fi', real(V(2,2)), imag(V(2,2)));
  end
  infostr{10,1} = ['     ',eig2_str,'    (',v2_1_str,', ',v2_2_str,')'];
...
elseif strcmp(action,'figdefault')
  ...
  switch style
   case 'black'
    ...
    % whitebg replaced with direct set commands for MATLAB 2025 compatibility
    if isunix | isvms
      fc = [.35 .35 .35];
    else
      fc = [.2 .2 .2];
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'user',dud);
   % Disabled cursor position display: 'WindowButtonMotionFcn','pplane8(''cdisp'')'
   set(disph,'WindowButtonDownFcn','pplane8(''down'')',...
      'WindowButtonUpFcn','');
   pplane8('dirfield',disph);
   ppset = findobj('name','pplane8 Setup');
